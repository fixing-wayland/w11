...
| **sway** (wlroots) | comes back in full, every output | lost, unless `--persistent` was used | nothing on disk from an IPC apply; only the sway config sway reads makes a layout stick, so `--persistent` writes `~/.config/sway/w11-outputs.conf` and makes that config `include` it (route 2). Measured on `swaytest`, 2026-09-14: after a full reboot Virtual-2 came up 1280x1024@60.020 at 1920,0 from the file. Run as root against somebody else's session (ssh, sudo), the layout lands live and the file half is written *as the seated user*: a forked child drops to their uid (`w11common/asuser.py` — initgroups, setgid, setuid, `HOME` from passwd, `XDG_CONFIG_HOME` dropped) and runs the same write, so the file and the `include` land in their home owned by them, and a symlink planted in their config directory reaches only what they could already write. Measured 2026-09-17 with a throwaway account: root without the drop wrote `/etc/w11target/pwned` as root:root through a planted `w11-outputs.conf.new`; the dropped child got `EACCES` and wrote nothing; `tests/test_asuser_root.py` repeats both under sudo. What remains **not yet** here: the seated session's own `$XDG_CONFIG_HOME` — the drop rebuilds `~/.config` from passwd, so a user whose session set it elsewhere gets a file sway never reads; the route is rung 4, `SO_PEERCRED` on the IPC socket (measured: `pid uid gid` of the compositor) then `/proc/<pid>/environ`, at the cost of a root that can read the compositor's environ (a plain root shell can; this container's root, without `CAP_SYS_PTRACE`, got `EACCES`) and one read per apply. A second, older gap of the same family stays filed: the run's own state (`wxrandr-state.json`, `--newmode` and last modes) is still written into the caller's runtime dir, rung 2 with the same drop around the state save. |
...
| **Hyprland** | not measured on the rig yet | lost | `hyprland.conf` and the files it `source`s. `--persistent` writes `~/.config/hypr/w11-monitors.conf` and adds a `source =` line for it (route 2); writing the file is itself a reload, so the whole config is re-read (runtime `hyprctl keyword`s set since login reset). Measured on arch-hypr, 0.56.2, 2026-09-11. Run as root against somebody else's session (ssh, sudo), the layout lands live and the file half is written *as the seated user*: a forked child drops to their uid (`w11common/asuser.py` — initgroups, setgid, setuid, `HOME` from passwd, `XDG_CONFIG_HOME` dropped) and runs the same write, so the rules file and its `source =` line land in their home owned by them, and a symlink planted in their config directory reaches only what they could already write. Measured 2026-09-17 with a throwaway account: root without the drop wrote `/etc/w11target/pwned` as root:root through a planted `w11-outputs.conf.new`; the dropped child got `EACCES` and wrote nothing; `tests/test_asuser_root.py` repeats both under sudo. What remains **not yet** here: the seated session's own `$XDG_CONFIG_HOME` — the drop rebuilds `~/.config` from passwd, so a user whose session set it elsewhere gets a file Hyprland never reads; the route is rung 4, `SO_PEERCRED` on the IPC socket (measured: `pid uid gid` of the compositor) then `/proc/<pid>/environ`, at the cost of a root that can read the compositor's environ (a plain root shell can; this container's root, without `CAP_SYS_PTRACE`, got `EACCES`) and one read per apply. A second, older gap of the same family stays filed: the run's own state (`wxrandr-state.json`, `--newmode` and last modes) is still written into the caller's runtime dir, rung 2 with the same drop around the state save. On Hyprland the reload route — write, `source =`, `reload`, re-read, restore on failure — runs whole inside that child, which can talk to the socket because it is now its owner. |
