...
`w11common/` holds what more than one tool needs and nothing else does, in nine
...
`procs.py` (detached children), `distro.py` (the distribution family from
...
message said before), and `asuser.py` (one function run as another uid — the file half of
`--persistent` on a session that is not the caller's). It is a package rather than a corner
of `wdotool` because that list is exactly what the *display* tools use of it: they find a
session, they talk D-Bus and Wayland, and they never type a key, never open a window backend
...
| `xkbmap.py`, `keymap.py`, `us_keymap.py` | `test_xkbmap`, `test_keymap`, `test_layout_flag` | `tests/fixtures/keymaps/*.xkb`, each a byte-for-byte capture of what a compositor handed a client, from GNOME, sway and KWin — except the three `tests/fixtures/keymaps/README.md` names as compiled rather than captured, and the two GNOME Shell 50.5 files, which are `four_us_de_fr_gr.xkb` and `es.xkb` byte for byte, kept under the names of the sessions they stand for because no golden here can produce that session's own bytes |
...
| `w11common/asuser.py` | `test_asuser`, `test_asuser_root` | the real fork and pipe with the drop stubbed; then, under sudo only, a throwaway account made with `useradd` and a planted symlink |
...
| `hacks/display/gnome_overlap.py` + `gnome/w11-overlap@w11/` | `test_gnome_overlap`, `test_gnome_overlap_js`, `test_overlap_consent`, `test_overlap_force` | the same mock bus with a mock `org.gnome.Shell` and a mock overlap extension on it, so a whole `--unsafe-gnome-overlap` run happens in-process; plain `node` running the extension's own `rules.js` against `monitors_xml.py`; and, for the shipped type descriptions, `g-ir-compiler` plus GIRepository in a subprocess per namespace — the shipped typelib and a fresh compile of the checked-in `.gir` are compared by *meaning* (namespace, no shared library, every function name and C symbol, every record's size and field offsets), because g-ir-compiler 1.86 writes 17 different reserved words per typelib than the compiler that produced the checked-in files. The consent file re-runs every refusal in the first with an agreement recorded, and asserts from the source that the agreement is read after the last one; and, since `tests/test_gnome_overlap_js.py`, the shipped extension itself under node through the same gi:// stubs as the bridge, its guard chain and `_apply` driven against a scripted heap |
...
| `wmirror/` | `test_wmirror_cli`, `test_wmirror_lifetime`, `test_wmirror_live`, `test_wmirror_cinnamon` | a fake `wl-mirror` (`support.WL_MIRROR_STUB`), and the detach protocol driven for real; then a real headless sway (`swaymsg create_output` for the second head) with the same stub, where the supervisor's watch reads real zwlr_output_management events for the only time in the suite; the Cinnamon Eval programs against `FakeEval`, with the nine wl-mirror-measured boxes as the unit table for `core.scale_plan` |
