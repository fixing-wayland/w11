...
#: Where a route-6 patched labwc writes each view's rectangle for this backend to read. AGENTS.md route
#: 6: zwlr_foreign_toplevel_management_v1 carries no rectangle, so the shipped labwc_0.9.3-1w11.1 .deb
#: (src/foreign-toplevel/foreign.c) emits `pid\tx\ty\tw\th\tapp_id\ttitle` per view into
#: $XDG_RUNTIME_DIR/<this> on map, move/resize, unmap and title/app_id change (the join key), and
#: `_labwc_geometry` folds it onto the native toplevels the wire gives no
#: geometry for. Absent on an unpatched compositor, where every native row keeps the floor.
...
class WlrBackend(XPlaneViews, WindowBackend):
    ...
    def list(self) -> list[Window]:
        ...
        Getting a rectangle for the native half is NOT YET, and the reachable route is rung 6 -- a patched
        compositor emitting one geometry event per window, one patched labwc `.deb` covering the four labwc
        goldens (labwc, Budgie, Xfce-on-Wayland, LXQt-on-Wayland) -- and rung 3 on river, a
        ...
        NOT YET, the same reachable rung as the rectangle -- 6 (a patched compositor) and 3 on river --
        the same missing per-toplevel field, closed the same way, and rung 1 later if a foreign-toplevel
        protocol carries the pid.
        ...
        for w in wins:
            ...
            # route 6: a patched labwc writes each view's rectangle to a runtime file, joined here by
            # (app_id, title) -- the same pair the X-plane join uses, a tie keeping the floor. When it is
            # there the native toplevel answers its true rectangle; when it is not (an unpatched
            # compositor, or river, whose rung is 3) the row keeps the floor and latches
            # `geometry_is_floor`, the way backend_cosmic's `_rect` latches it.
    ...
    def _labwc_geometry(self):
        """The per-view rectangles a route-6 patched labwc writes to `$XDG_RUNTIME_DIR/w11-labwc-geometry`,
        keyed by `(app_id, title)` with any key two windows share dropped, or `{}` when there is no such file.

        AGENTS.md route 6. `zwlr_foreign_toplevel_management_v1` and `ext_foreign_toplevel_list_v1` carry no
        rectangle, so a NATIVE toplevel's geometry never reaches this backend over the wire and the X plane
        (route 5) reaches XWayland windows only. The shipped `labwc_0.9.3-1w11.1` .deb closes the native half
        by emitting the current `view->current` box -- the same rectangle `getwindowgeometry` prints for an
        XWayland view -- one line `pid\tx\ty\tw\th\tapp_id\ttitle` per view that carries a
        foreign-toplevel handle. The join is `(app_id, title)`, the same pair `match_xids` pairs the X plane
        on, and a key two windows share is dropped so a tie keeps the floor rather than guess -- exactly the
        X-plane rule from the other end. A missing file (an unpatched compositor, sway/Wayfire before their
        Xwayland, or river, whose rung is 3 not 6) yields `{}` and every native row keeps the floor."""
        try:
